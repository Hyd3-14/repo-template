...
Dependabot minor and patch updates may be auto-merged only after the `Dependabot auto-merge eligibility`, `Baseline static checks`, and `GitHub Actions Static Checks` checks pass; major updates remain manual review and marking the pull request as draft keeps it on manual hold.
